...
PACKAGE_NAME="magiwol_${RELEASE_VERSION_MAJOR}.${RELEASE_VERSION_MINOR}_${RELEASE_ARCHITECTURE}"
...
mkdir -p "$DIRECTORY_PACKAGE/opt/magiwol"
unzip -LL "$PATH_RELEASE" -d "$DIRECTORY_PACKAGE/opt/magiwol" > /dev/null
if [ ! $? -eq 0 ]
then
    echo "Cannot extract archive!" >&2
    exit 1
fi
chmod 755 "$DIRECTORY_PACKAGE/opt/magiwol/magiwol.exe"
